...
ONE.isTouch = ('ontouchstart' in window) || (navigator.maxTouchPoints > 0);
document.documentElement.classList.add(ONE.isTouch ? 'touch' : 'no-touch');
...
document.addEventListener('DOMContentLoaded', () => {
	document.querySelectorAll('form:not([data-form_type="search"])').forEach(form => {
		form.addEventListener('submit', function(event) {
			...
			currentUrl.searchParams.set('dwwform', 'succes');
